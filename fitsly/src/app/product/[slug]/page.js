    ...
    const fetchProduct = async () => {
      try {
        ...
        if (res.data.success) {
          ...
          const colors = [...new Set(res.data.product.inventory.map(i => i.color))];
          const sizes = [...new Set(res.data.product.inventory.map(i => i.size))];
        ...
        <div className="lg:w-4/5 mx-auto flex flex-wrap">
          ...
          <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
            ...
            <div className="flex mt-6 items-center pb-5 border-b-2 border-gray-100 mb-5">
              <div className="flex">
                ...
                {colors.map((color) => (
                  <button
                    key={color}
                    className={`border-2 ml-1 rounded-full w-6 h-6 focus:outline-none ${selectedColor === color ? 'border-pink-600' : 'border-gray-300'}`}
            ...
            <div className="flex">
              ...
              <button onClick={() => buyNow(product._id, 1, product.price, product.title, selectedSize, selectedColor)} className="flex ml-8 text-white bg-pink-500 border-0 py-2 px-2 md:px-6 focus:outline-none hover:bg-pink-600 rounded">Buy Now</button>
              <button
                onClick={() => addToCart(product._id, 1, product.price, product.title, selectedSize, selectedColor)}
